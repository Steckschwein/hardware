...
/*========================================================= SFX Sound Expander =================================================
  Variables
*/
.var fm_file_base_address = $0002		// word
.var fm_file_arrdata = $0008			// word
...
.pc = $1003 "PLAYER" 
		jmp jch_fm_play_
fm_data_st_offset: .word 0							// location of FM file (D00), any caller must set $1006/$1007 first with the location of the D00 (lowbyte/highbyte)
// ----------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------
fm_voice_has_data: .fill 9,0				// 0 = no data, no playing; 1 = data, process playing. per voice
...
fm_voice_instrument: .fill 18,0				// 6D5 = current channel/voice instrument (effect cxxxx)
fm_cur_seq_command_tick_counter: .fill 18,0	// current sequence command time/tick counter (per channel/voice)	
fm_voice_value: .fill 18, 0					// 6f9 = effect 6 (cut voice), 9 voices, 9 words, but also others store value here
fm_position_in_current_seq: .fill 18, 0		// the position in the current sequence	
fm_position_in_voice_seqlist: .fill 18, 0	// position counter for the sequence in the sequence list
fm_pt_voice_arrdata: .fill 18, 0			// pointers to start of arrangement data per channel/voice (words, 9)
fm_current_frequency: .fill 18, 0			// fm_current_frequency
fm_transpose_value: .fill 18, 0				// fm_transpose_value for sequence by voice
fm_voice_temp_slide: .fill 18, 0			// temporary slide valure storage by voice
var_777: .fill 9,0							// what is this doing?
fm_cur_vibrato_speed_counter: .fill 9,0		// counter to keep track of the speed of the vibrato
fm_voice_vibrato_speed: .fill 9,0			// the vibrato speed of the voice
fm_voice_vibrato_depth: .fill 9,0			// the vibrato depth of the voice
fm_ins_multipurpose: .fill 18, 0			// value for the multipurpose register of the OPL by voice
var_7ad: .fill 9,0							// what is this doing?
fm_ins_fine_tune: .fill 9,0					// fine tune value per voice
fm_temp_ins_hardrestart: .fill 9,0			// temp hard restart value by voice
fm_ins_hardrestart: .fill 9,0				// instrument hard restart value (number of ticks/time units/calls to player) before start
fm_voice_vibrato: .fill 18, 0				// 7D1 = current channel/voice vibrato (effect 7xxx)
fm_voice_slide: .fill 18, 0					// 7E3 = current channel/voice slide (effect Dxxx)
fm_voice_freq: .fill 18, 0					// 7F5 = current freq for voice?
fm_voice_volume: .fill 9, 0					// 807 = current channel/voice volume (effect 9xxx)
fm_current_channel_speed_counter: .fill 9, 0	// current channel speed counter
fm_channel_speed_counter: .fill 9, 0		// channel speed table (bytes), one for each voice/channel (9)
fm_previous_registry_value: .fill 9, 0			// 822 : previous register value
fm_previous_registry_value_preset: .fill 9, 0	// 82b : previous register value preset
fm_channel_volume: .fill 36, 0				// channel volume (3f = silence)
fm_cur_voice_volume: .fill 32, 0				// current colume of the voice
fm_ins_mainvolume_ksl: .fill 32, 0				// instrument register value for volume/ksl
fm_ins_modulator_level: .fill 32, 0				// instrument register modulator level 
fm_ins_feedback_level: .fill 52, 0				// instrument register feedback level
fm_minimum_voice_volume: .fill 9, 0			// temporary voice level/volume
fm_sequence_var_type: .fill 35, 0				// 01 = 7e(hold)/0(rest), 02 = Effect, 00 = standard note
...
fm_frequency_table: .byte 87, 1, 107, 1, 129, 1, 152, 1, 176, 1, 202, 1, 229, 1, 2, 2, 32, 2, 65, 
   2, 99, 2, 135, 2, 87, 5, 107, 5, 129, 5, 152, 5, 176, 5, 202, 5, 229, 5,
   2, 6, 32, 6, 65, 6, 99, 6, 135, 6, 87, 9, 107, 9, 129, 9, 152, 9, 176,
   9, 202, 9, 229, 9, 2, 10, 32, 10, 65, 10, 99, 10, 135, 10, 87, 13, 107, 13,
   129, 13, 152, 13, 176, 13, 202, 13, 229, 13, 2, 14, 32, 14, 65, 14, 99, 14, 135,
   14, 87, 17, 107, 17, 129, 17, 152, 17, 176, 17, 202, 17, 229, 17, 2, 18, 32, 18,
   65, 18, 99, 18, 135, 18, 87, 21, 107, 21, 129, 21, 152, 21, 176, 21, 202, 21, 229,
   21, 2, 22, 32, 22, 65, 22, 99, 22, 135, 22, 87, 25, 107, 25, 129, 25, 152, 25,
   176, 25, 202, 25, 229, 25, 2, 26, 32, 26, 65, 26, 99, 26, 135, 26, 87, 29, 107,
   29, 129, 29, 152, 29, 176, 29, 202, 29, 229, 29, 2, 30, 32, 30, 65, 30, 99, 30,
   135, 30, 0
fm_opt_ins_pointers: .fill 64, 0 			// optimization: 32 16-bit pointers to instrument data
fm_opt_seq_pointers: .fill 128, 0 			// optimization: 64 16-bit pointers to sequence data
...
tpoint2: .word 0 
tword1: .word 0
message: .text "edlib player v0.2 (somewhat optimized) by mr.mouse/xentax july 2017@"
